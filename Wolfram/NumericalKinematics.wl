...
OrthogonalTwistor[twi1_List,twi2_List,twi3_List]:=Table[Sum[If[DuplicateFreeQ[{i,j,k,l}],Signature[{i,j,k,l}],0]*twi1[[j]]*twi2[[k]]*twi3[[l]],{j,4},{k,4},{l,4}],{i,4}]

FourBracket[twi1_List,twi2_List,twi3_List,twi4_List]:=Sum[If[DuplicateFreeQ[{i,j,k,l}],Signature[{i,j,k,l}],0]*twi1[[i]]*twi2[[j]]*twi3[[k]]*twi4[[l]],{i,4},{j,4},{k,4},{l,4}]

DualTwistors[twistors_]:=Table[OrthogonalTwistor[twistors[[Mod[i-1,#,1]]],twistors[[Mod[i,#,1]]],twistors[[Mod[i+1,#,1]]]]/(TwoBracket[twistors[[Mod[i-1,#,1]]],twistors[[Mod[i,#,1]]]]*TwoBracket[twistors[[Mod[i,#,1]]],twistors[[Mod[i+1,#,1]]]]),{i,#}]&@Length[twistors]


PlanarKinematics[twistors_]:=Table[FourBracket[twistors[[Mod[i-1,#,1]]],twistors[[Mod[i,#,1]]],twistors[[Mod[i+1,#,1]]],twistors[[Mod[i+2,#,1]]]]/(TwoBracket[twistors[[Mod[i-1,#,1]]],twistors[[Mod[i,#,1]]]]*TwoBracket[twistors[[Mod[i+1,#,1]]],twistors[[Mod[i+2,#,1]]]]),{i,#}]&@Length[twistors]


CheckKinematics[twistors_List]:=
If[MemberQ[#,0],True,False]&@
...
twistors=ReplacePart[twistors,Thread[Rule[#,Table[x[i],{i,Length[#]}]]]]&@(Prepend[{2,4},#]&/@Flatten[Drop[#,1]&/@OptionValue["Identical"]]);

duals=If[Length[#]==2,#[[1]],0]&/@FoldPairList[TakeDrop,PlanarKinematics[Sequence@@@twistors],massive]; (*the masses*)

duals=Flatten@Solve[Equal@@@Map[duals[[#]]&,Flatten[Subsequences[#,{2}]&/@OptionValue["Identical"],1],{2}]];
...
If[
CheckKinematics[Sequence@@@duals]&&n>3,

GenerateKinematics[n,"Masses"->OptionValue["Masses"],"Identical"->OptionValue["Identical"],"HighestNumber"->OptionValue["HighestNumber"]],

x=If[OptionValue["Labels"]=="Numeric",Range@n,OptionValue["Labels"]];
...
Protect@@DeleteCases[Names["NumericalKinematics`*"],_?(MatchQ[#,Alternatives@@(ToString/@{NSpinorDottedML,NSpinorUndottedML,NSpinorDottedMV,NSpinorUndottedMV,NMomentum})]&)]
